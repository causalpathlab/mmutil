...
if ! [ -f example/brain_2018-05-03/temp.u.gz ] ; then
	mmutil_spectral_col --mtx data/brain_2018-05-03/filtered_count_matrix.mtx.gz \
       --rank 50 --log_scale --sampling_method CV --col_norm 100000 --out example/brain_2018-05-03/temp 
fi
```
...
if ! [ -f example/brain_2018-05-03/gmm.argmax.gz ]; then
	...
	zcat example/brain_2018-05-03/temp.u.gz | cut -f 2-20 -d ' ' | gzip > example/brain_2018-05-03/temp.20.u.gz

    mmutil_spectral_cluster_col --sdata example/brain_2018-05-03/temp.20.u.gz \
...
if ! [ -f example/brain_2018-05-03/dbscan_level_1.argmax.gz ]; then
    ...
    mmutil_spectral_cluster_col --sdata example/brain_2018-05-03/temp.20.u.gz \
      --method DBSCAN --knn 100 --nlist 100 --bilink 10 --epsilon 5.0 \
      --num_levels 10 --min_size 5 --embedding_epochs 50 \
	  --verbose --out example/brain_2018-05-03/dbscan 2> /dev/null

fi
```

Read and show DBSCAN estimation at multiple resolutions:

```{r fig_scatter_dbscan}
for(j in 1:10){
    .show.scatter(out.dir %&% "/dbscan_level_" %&% j %&% ".argmax.gz",
                  out.dir %&% "/dbscan_level_" %&% j %&% ".embedded.gz")
}
```

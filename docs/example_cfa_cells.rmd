...
meta.dt <- fread("data/brain_2018-05-03/filtered_column_metadata.txt.gz", header=TRUE)
pheno.dt <- fread("data/brain_2018-05-03/phenotypes.csv", header=TRUE, na.strings = "-9")
clin.dt <- fread("data/brain_2018-05-03/ROSMAP_clinical.csv", header=TRUE)
annot.dt <- .fread("example/cfa/psych.annot.gz",
                   col.names = c("TAG", "celltype", "prob", "ln.prob"))
...
clin.dt[, age.death := sapply(age_death, .age.code)]
clin.dt[, apoe.e4 := sapply(apoe_genotype, .apoe.code)]

pheno.dt <- pheno.dt %>%
    select(projid, pmAD, pathoAD, np_sqrt, nft_sqrt)

pheno.dt <- pheno.dt %>% 
    left_join(clin.dt, by = "projid")

################################################
## these are cells with annotated individuals ##
################################################

meta.pheno.dt <- annot.dt %>%
    left_join(meta.dt) %>%
    left_join(pheno.dt) %>%
...
if(!file.exists(.tag.file)) {
    ...
    ## make sure that we use the cells with matching annotations
    .mathys <- c("Ast", "End", "Ex", "In", "Mic", "Oli", "Opc", "Per")
    .encode <- c("Astro", "Endo", "Ex", "In", "Microglia", "Oligo", "OPC", "Per")

    .out.dt <- meta.pheno.dt %>% 
        mutate(ct.mathys = factor(broad.cell.type, .mathys, .encode)) %>%
        mutate(ct.mathys = as.character(ct.mathys)) %>%
        filter(ct.mathys == celltype)

    .fwrite(.out.dt[, .(TAG)], .tag.file)
}
```
...
if(any(!file.exists(.trt.file, .annot.file, .ind.file, .lab.file))) {
    ...
    y.pheno <- cols.dt %>%
        mutate(y = if_else(pathoAD > 0, "AD", "HC")) %>%
        select(y)
...
[ -f example/cfa/output.resid_mu.gz ] || \
	OMP_NUM_THREADS=8 ../src/mmutil_cfa_col \
		--mtx example/cfa/input.mtx.gz \
		--col example/cfa/input.cols.gz \
		--annot example/cfa/input.annot.gz \
		...
		--lab example/cfa/input.lab.gz \
		--ind example/cfa/input.ind.gz \
		--a0 1 --b0 1 --glm_pseudo 1 \
		--knn 30 --rank 30 --log_scale --nboot 100 \
		--out example/cfa/output
```
...
```{r helper_functions_to_organize_results}
...
.read.stat.all <- function(.hdr, ...) {

    .col.file <- .hdr %&% ".mu_cols.gz"
    .col.sep <- c("projid", "disease", "celltype")

    .dt1 <- .read.stat(.hdr %&% ".boot_ln_mu.gz", "ln.mu", .col.file, ...)
    .dt2 <- .read.stat(.hdr %&% ".boot_sd_ln_mu.gz", "ln.mu.sd", .col.file, ...)
    ...
    .dt7 <- .read.stat(.hdr %&% ".boot_ln_internal_mu.gz", "ln.mu.intern", .col.file, ...)
    .dt8 <- .read.stat(.hdr %&% ".boot_sd_ln_internal_mu.gz", "ln.mu.intern.sd", .col.file, ...)
    ...
    ret <- .dt1 %>%
        ...
        merge(.dt7, by=c("gene", "col")) %>% 
        merge(.dt8, by=c("gene", "col"))
...
if(!file.exists(.data.file)) {
    stat.tab <- .read.stat.all("example/cfa/output")
...
test.gene.association <- function(.ln.mat) {
    ...
    .take.gene.assoc <- function(ct,
                                 .ln.mat,
                                 .row.df,
                                 .pheno = c("age.death", "msex", "braaksc", "apoe.e4")){
...
```{r fig_confounding_effects_tsne, fig.width=4, fig.height=6}
.row.df <- match.row.with.pheno(.ln.mat)
...
.gg.save(filename = .file, plot = plt, width=4, height=6)
print(plt)
```

...
struct row_name_vec_t {
  ...
  explicit row_name_vec_t(type &_val) : val(_val) {}
  type &val;
};

struct col_name_vec_t {
  using type = std::vector<std::string>;
  explicit col_name_vec_t(type &_val) : val(_val) {}
  type &val;
};

struct row_index_map_t {
  using type = std::unordered_map<std::string, std::ptrdiff_t>;
  explicit row_index_map_t(type &_val) : val(_val) {}
  type &val;
};

struct col_index_map_t {
  using type = std::unordered_map<std::string, std::ptrdiff_t>;
  explicit col_index_map_t(type &_val) : val(_val) {}
  type &val;
};
...
read_named_eigen_sparse_stream(IFS &ifs,                       //
  ...
  auto insert_triplet = [&]() {
    ...
    triplets.push_back(Triplet(i_index.at(i_str), j_index.at(j_str), weight));
  };
...
read_named_membership_file(const std::string membership_file,    //
  ...
  Index kk = 0;
  for (auto pp : pi_jk) {
    if (j_index.count(std::get<0>(pp)) == 0) continue;
  ...
  for (auto pp : pi_jk) {
    if (j_index.count(std::get<0>(pp)) == 0) continue;
    if (k_index.count(std::get<1>(pp)) == 0) continue;

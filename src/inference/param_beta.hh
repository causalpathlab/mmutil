...
struct param_beta_t {
  ...
  typedef param_tag_beta sgd_tag;
  ...
  struct pr_one_minus_pr_t {
    inline const scalar_t operator()(const scalar_t& pr) const { return pr * (one_val - pr); }
  } pr_one_minus_pr_op;
...
make_dense_beta(const Index n1, const Index n2, const Opt& opt) {
  ...
  using Param = param_beta_t<Mat, param_tag_dense>;

  Param ret(n1, n2, opt);
  param_impl_initialize(ret, param_tag_beta());
  param_resolve(ret);
...
void
param_impl_initialize(Parameter& P, const param_tag_beta) {
...
make_sparse_beta(const Eigen::SparseMatrixBase<Derived>& A, const Opt& opt) {
  ...
  using Param = param_beta_t<Mat, param_tag_sparse>;
...
void
param_impl_update_sgd(Parameter& P, const scalar_t rate, const param_tag_beta) {
...
template <typename Parameter, typename scalar_t>
void
hyperparam_impl_update_sgd(Parameter& P, const scalar_t rate, const param_tag_beta) {
  ;  // nothing to do
}
...
template <typename Parameter>
void
param_impl_resolve(Parameter& P, const param_tag_beta) {
...
template <typename Parameter>
void
hyperparam_impl_resolve(Parameter& P, const param_tag_beta) {
  ;  // nothing to do
}

template <typename Parameter, typename scalar_t, typename RNG>
void
param_impl_perturb(Parameter& P, const scalar_t sd, RNG& rng, const param_tag_beta) {
...
template <typename Parameter, typename scalar_t>
void
param_impl_perturb(Parameter& P, const scalar_t sd, const param_tag_beta) {
  std::mt19937 rng;
  param_impl_perturb(P, sd, rng, param_tag_beta());
}

template <typename Parameter>
void
param_impl_check_nan(Parameter& P, const param_tag_beta) {
...
template <typename Parameter>
const auto&
param_impl_mean(Parameter& P, const param_tag_beta) {
  return P.theta;
}

template <typename Parameter>
const auto&
param_impl_var(Parameter& P, const param_tag_beta) {
  return P.theta_var;
}
...
param_impl_eval_sgd(Parameter& P, const M1& G1, const M2& G2, const M3& Nobs,
                    const param_tag_beta) {
  // gradient w.r.t. beta
  P.grad_beta = G1.cwiseProduct(P.theta.unaryExpr(P.pr_one_minus_pr_op)).cwiseQuotient(Nobs);

  // gradient w.r.t. phi_aux
  P.grad_phi_aux =
      -G2.cwiseProduct(P.theta_var.unaryExpr(P.pr_one_minus_pr_op)).cwiseQuotient(Nobs);
}
...
template <typename Parameter>
void
param_impl_write(Parameter& P, const std::string hdr, const std::string gz, const param_tag_beta) {

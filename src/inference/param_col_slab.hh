...
struct param_col_slab_t {
  ...
  typedef param_tag_col_slab sgd_tag;
...
void
param_impl_initialize(Parameter& P, const param_tag_col_slab) {
...
auto
make_dense_col_slab(const Index n1, const Index n2, const Opt& opt) {
  using Mat   = Eigen::Matrix<Scalar, Eigen::Dynamic, Eigen::Dynamic>;
  using Param = param_col_slab_t<Mat, param_tag_dense>;
  Param ret(n1, n2, opt);
  param_impl_initialize(ret, param_tag_col_slab());
  param_resolve(ret);
...
template <typename Scalar, typename Index, typename Opt>
auto
make_dense_col_slab_ptr(const Index n1, const Index n2, const Opt& opt) {
  using Mat    = Eigen::Matrix<Scalar, Eigen::Dynamic, Eigen::Dynamic>;
  using Param  = param_col_slab_t<Mat, param_tag_dense>;
...
auto
make_sparse_col_slab(const Eigen::SparseMatrixBase<Derived>& A, const Opt& opt) {
  const auto n1 = A.rows();
  const auto n2 = A.cols();

  using Mat   = Eigen::SparseMatrix<Scalar, Eigen::ColMajor>;
  using Param = param_col_slab_t<Mat, param_tag_sparse>;
  Param ret(n1, n2, opt);
...
template <typename Parameter>
void
param_impl_resolve(Parameter& P, const param_tag_col_slab) {
...
template <typename Parameter>
void
hyperparam_impl_resolve(Parameter& P, const param_tag_col_slab) {
...
void
param_impl_perturb(Parameter& P, const Scalar sd, const param_tag_col_slab) {
  std::mt19937 rng;
...
void
param_impl_perturb(Parameter& P, const Scalar sd, RNG& rng, const param_tag_col_slab) {
  std::normal_distribution<Scalar> Norm;
  auto rnorm = [&rng, &Norm, &sd](const auto& x) { return sd * Norm(rng); };
...
template <typename Parameter>
const auto&
param_impl_mean(Parameter& P, const param_tag_col_slab) {
  return P.beta;
}

template <typename Parameter>
const auto&
param_impl_var(Parameter& P, const param_tag_col_slab) {
  return P.theta_var;
}
...
param_impl_eval_sgd(Parameter& P, const M1& G1, const M2& G2, const M3& Nobs,
  ...
  // chain rule
  P.grad_gamma_aux = P.grad_gamma_aux.cwiseProduct(P.gamma_aux.unaryExpr(P.grad_gamma_chain_op));
...
template <typename Parameter, typename Scalar>
void
param_impl_update_sgd(Parameter& P, const Scalar rate, const param_tag_col_slab) {
...
template <typename Parameter, typename Scalar>
void
hyperparam_impl_update_sgd(Parameter& P, const Scalar rate, const param_tag_col_slab) {
...
param_impl_write(Parameter& P, const std::string hdr, const std::string gz,
  ...
  TLOG("Tau = " << std::setw(10) << P.tau_val << " [" << std::setw(10) << P.tau_aux << "]");
}

template <typename Parameter>
void
param_impl_check_nan(Parameter& P, const param_tag_col_slab) {

...
struct param_col_slab_zero_t {
  ...
  typedef param_tag_col_slab_zero sgd_tag;
  typedef S sparsity_tag;

  using Dense = Eigen::Matrix<scalar_t, Eigen::Dynamic, Eigen::Dynamic>;

  template <typename Opt>
  explicit param_col_slab_zero_t(const index_t n1, const index_t n2, const Opt& opt)
      : nrow(n1),
        ncol(n2),
        onesN1(1, n1),
        beta(nrow, ncol),
...
void
param_impl_initialize(Parameter& P, const param_tag_col_slab_zero) {
...
make_dense_col_slab_zero(const Index n1, const Index n2, const Opt& opt) {
  ...
  using Param = param_col_slab_zero_t<Mat, param_tag_dense>;
  Param ret(n1, n2, opt);
  param_impl_initialize(ret, param_tag_col_slab_zero());
  param_resolve(ret);
...
auto
make_sparse_col_slab_zero(const Eigen::SparseMatrixBase<Derived>& A, const Opt& opt) {
  const auto n1 = A.rows();
  const auto n2 = A.cols();

  using Mat   = Eigen::SparseMatrix<Scalar, Eigen::ColMajor>;
  using Param = param_col_slab_zero_t<Mat, param_tag_sparse>;
  Param ret(n1, n2, opt);
...
template <typename Parameter>
void
param_impl_resolve(Parameter& P, const param_tag_col_slab_zero) {
...
template <typename Parameter>
void
hyperparam_impl_resolve(Parameter& P, const param_tag_col_slab_zero) {
...
void
param_impl_perturb(Parameter& P, const Scalar sd, const param_tag_col_slab_zero) {
  // do nothing
  param_resolve(P);
}
...
template <typename Parameter>
const auto&
param_impl_mean(Parameter& P, const param_tag_col_slab_zero) {
  return P.beta;
}

template <typename Parameter>
const auto&
param_impl_var(Parameter& P, const param_tag_col_slab_zero) {
  return P.theta_var;
}
...
template <typename Parameter, typename Scalar>
void
param_impl_update_sgd(Parameter& P, const Scalar rate, const param_tag_col_slab_zero) {
...
template <typename Parameter, typename Scalar>
void
hyperparam_impl_update_sgd(Parameter& P, const Scalar rate, const param_tag_col_slab_zero) {
...
template <typename Parameter>
void
param_impl_check_nan(Parameter& P, const param_tag_col_slab_zero) {

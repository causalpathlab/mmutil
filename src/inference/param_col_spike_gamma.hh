...
struct param_col_spike_gamma_t {
  ...
  typedef param_tag_col_spike_gamma sgd_tag;
  typedef S sparsity_tag;

  using Dense = Eigen::Matrix<scalar_t, Eigen::Dynamic, Eigen::Dynamic>;

  template <typename Opt>
  explicit param_col_spike_gamma_t(const index_t n1, const index_t n2, const Opt& opt)
      : nrow(n1),
        ncol(n2),
        onesN1(1, n1),
        alpha(nrow, ncol),
  ...
  struct grad_alpha_g2_t {
    ...
    inline const scalar_t operator()(const scalar_t& a, const scalar_t& m) const {
  ...
  struct grad_beta_chain_rule_t {
    inline const scalar_t operator()(const scalar_t& x, const scalar_t& b) const {
      if (-b > large_exp_value) {
...
void
param_impl_initialize(Parameter& P, const param_tag_col_spike_gamma) {
...
make_dense_col_spike_gamma(const Index n1, const Index n2, const Opt& opt) {
  ...
  using Param = param_col_spike_gamma_t<Mat, param_tag_dense>;

  Param ret(n1, n2, opt);
  param_impl_initialize(ret, param_tag_col_spike_gamma());
  param_resolve(ret);
...
auto
make_sparse_col_spike_gamma(const Eigen::SparseMatrixBase<Derived>& A, const Opt& opt) {
  const auto n1 = A.rows();
  const auto n2 = A.cols();

  using Mat   = Eigen::SparseMatrix<scalar_t, Eigen::ColMajor>;
  using Param = param_col_spike_gamma_t<Mat, param_tag_sparse>;
...
void
param_impl_update_sgd(Parameter& P, const scalar_t rate, const param_tag_col_spike_gamma) {
...
template <typename Parameter, typename scalar_t>
void
hyperparam_impl_update_sgd(Parameter& P, const scalar_t rate, const param_tag_col_spike_gamma) {
...
template <typename Parameter>
void
param_impl_resolve(Parameter& P, const param_tag_col_spike_gamma) {
...
template <typename Parameter>
void
hyperparam_impl_resolve(Parameter& P, const param_tag_col_spike_gamma) {
...
void
param_impl_perturb(Parameter& P, const scalar_t sd, RNG& rng, const param_tag_col_spike_gamma) {
...
template <typename Parameter, typename scalar_t>
void
param_impl_perturb(Parameter& P, const scalar_t sd, const param_tag_col_spike_gamma) {
  std::mt19937 rng;
  param_impl_perturb(P, sd, rng, param_tag_col_spike_gamma());
}

template <typename Parameter>
void
param_impl_check_nan(Parameter& P, const param_tag_col_spike_gamma) {
...
template <typename Parameter>
const auto&
param_impl_mean(Parameter& P, const param_tag_col_spike_gamma) {
  return P.theta;
}

template <typename Parameter>
auto
param_impl_log_odds(Parameter& P, const param_tag_col_spike_gamma) {
  return P.alpha_aux_col;
}

template <typename Parameter>
const auto&
param_impl_var(Parameter& P, const param_tag_col_spike_gamma) {
  return P.theta_var;
}
...
param_impl_eval_sgd(Parameter& P, const M1& G1, const M2& G2, const M3& Nobs,
                    const param_tag_col_spike_gamma) {
  ////////////////////////////////////////////////////////////////
  ...
  P.grad_beta =
      G1.cwiseProduct(P.alpha) + G2.cwiseProduct(P.alpha.binaryExpr(P.beta, P.grad_mu_g2_op));

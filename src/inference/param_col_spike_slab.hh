#ifndef PARAM_COL_SPIKE_SLAB_EB_HH_
#define PARAM_COL_SPIKE_SLAB_EB_HH_

template <typename T, typename S>
struct param_col_spike_slab_t {
  typedef T data_t;
  typedef typename T::Scalar scalar_t;
  typedef typename T::Index index_t;
  typedef adam_t<T, scalar_t> grad_adam_t;
  typedef param_tag_col_spike_slab sgd_tag;
  typedef S sparsity_tag;

  using Dense = Eigen::Matrix<scalar_t, Eigen::Dynamic, Eigen::Dynamic>;

  template <typename Opt>
  explicit param_col_spike_slab_t(const index_t n1, const index_t n2, const Opt& opt)
      : nrow(n1),
        ncol(n2),
        onesN1(1, n1),
        alpha(nrow, ncol),
...
void
param_impl_initialize(Parameter& P, const param_tag_col_spike_slab) {
...
auto
make_dense_col_spike_slab(const Index n1, const Index n2, const Opt& opt) {
  using Mat   = Eigen::Matrix<Scalar, Eigen::Dynamic, Eigen::Dynamic>;
  using Param = param_col_spike_slab_t<Mat, param_tag_dense>;

  Param ret(n1, n2, opt);
  param_impl_initialize(ret, param_tag_col_spike_slab());
  param_resolve(ret);
...
template <typename Scalar, typename Index, typename Opt>
auto
make_dense_col_spike_slab_ptr(const Index n1, const Index n2, const Opt& opt) {
  using Mat   = Eigen::Matrix<Scalar, Eigen::Dynamic, Eigen::Dynamic>;
  using Param = param_col_spike_slab_t<Mat, param_tag_dense>;
...
auto
make_sparse_col_spike_slab(const Eigen::SparseMatrixBase<Derived>& A, const Opt& opt) {
  const auto n1 = A.rows();
  const auto n2 = A.cols();

  using Mat   = Eigen::SparseMatrix<Scalar, Eigen::ColMajor>;
  using Param = param_col_spike_slab_t<Mat, param_tag_sparse>;
  ...
  initialize(Acol, ret.grad_gamma_aux_col, eps);
...
void
param_impl_update_sgd(Parameter& P, const Scalar rate, const param_tag_col_spike_slab) {
  ...
  P.alpha_aux_col += update_adam(P.adam_alpha_aux_col, P.grad_alpha_aux_col) * rate;
  P.gamma_aux_col += update_adam(P.adam_gamma_aux_col, P.grad_gamma_aux_col) * rate;
...
template <typename Parameter, typename Scalar>
void
hyperparam_impl_update_sgd(Parameter& P, const Scalar rate, const param_tag_col_spike_slab) {
...
template <typename Parameter>
void
param_impl_resolve(Parameter& P, const param_tag_col_spike_slab) {
...
template <typename Parameter>
void
hyperparam_impl_resolve(Parameter& P, const param_tag_col_spike_slab) {
...
template <typename Parameter, typename Scalar>
void
param_impl_perturb(Parameter& P, const Scalar sd, const param_tag_col_spike_slab) {
  std::mt19937 rng;
...
void
param_impl_perturb(Parameter& P, const Scalar sd, RNG& rng, const param_tag_col_spike_slab) {
  std::normal_distribution<Scalar> Norm;
  auto rnorm = [&rng, &Norm, &sd](const auto& x) { return sd * Norm(rng); };
...
template <typename Parameter>
void
param_impl_check_nan(Parameter& P, const param_tag_col_spike_slab) {
...
template <typename Parameter>
const auto&
param_impl_mean(Parameter& P, const param_tag_col_spike_slab) {
  return P.theta;
}

template <typename Parameter>
auto
param_impl_log_odds(Parameter& P, const param_tag_col_spike_slab) {
  return P.alpha_aux_col;
}

template <typename Parameter>
const auto&
param_impl_var(Parameter& P, const param_tag_col_spike_slab) {
  return P.theta_var;
}
...
param_impl_eval_sgd(Parameter& P, const M1& G1, const M2& G2, const M3& Nobs,
                    const param_tag_col_spike_slab) {
  ////////////////////////////////

...
struct param_mixture_t {
  ...
  typedef param_tag_mixture sgd_tag;
  ...
  struct resolve_prior_var_op_t {
    explicit resolve_prior_var_op_t(const scalar_t _gammax, const scalar_t& _tau_aux)
        : vmin(1 / _gammax),
          tau_aux(_tau_aux) {}
  ...
  struct grad_kl_lodds_alpha_t {
    ...
    inline const scalar_t operator()(const scalar_t& x) const { return lodds - x; }
    const scalar_t& lodds;
  } grad_alpha_lo_op;

  // (1 - 2 * a) * b^2
  struct grad_alpha_g2_t {
    inline const scalar_t operator()(const scalar_t& a, const scalar_t& b) const {
  ...
  struct grad_ln_ratio_alpha_t {
    explicit grad_ln_ratio_alpha_t(const scalar_t& _v1, const scalar_t& _v0) : v1(_v1), v0(_v0) {}
    inline const scalar_t operator()(const scalar_t& w1, const scalar_t& w0) const {
  ...
  struct grad_alpha_chain_rule_t {
    inline const scalar_t operator()(const scalar_t& x, const scalar_t& a) const {
  ...
  struct grad_kl_gamma_t {
    explicit grad_kl_gamma_t(const scalar_t& _v1, const scalar_t& _v0) : v1(_v1), v0(_v0) {}
  ...
  struct grad_kl_gamma_null_t {
    ...
    inline const scalar_t operator()(const scalar_t& a, const scalar_t& w0) const {
  ...
  struct grad_gamma_chain_rule_t {
    explicit grad_gamma_chain_rule_t(const scalar_t _gammax, const scalar_t& _tau_aux)
        : vmin(1 / _gammax),
          tau_aux(_tau_aux) {}
  ...
  grad_gamma_chain_rule_t grad_gamma_chain_null_op;  // grad_gamma_op(gammax,tau_null_aux)
  ...
  struct grad_prior_pi_aux_t {
    ...
    inline const scalar_t operator()(const scalar_t& a) const { return a - pi_val; }
    const scalar_t& pi_val;
  } grad_prior_pi_op;

  // 0.5 * alpha * (theta_sq/(v1 + v0)^2 - 1/(v1 + v0))
  struct grad_kl_tau_t {
    explicit grad_kl_tau_t(const scalar_t& _v1, const scalar_t& _v0) : v1(_v1), v0(_v0) {}

    inline const scalar_t operator()(const scalar_t& a, const scalar_t& ts) const {
  ...
  struct grad_tau_chain_rule_t {
    explicit grad_tau_chain_rule_t(const scalar_t _gammax) : vmin(1 / _gammax) {}
...
void
param_impl_initialize(Parameter& P, const param_tag_mixture) {
  // zero gradients
...
auto
make_dense_mixture(const Index n1, const Index n2, const Opt& opt) {
  using Mat   = Eigen::Matrix<scalar_t, Eigen::Dynamic, Eigen::Dynamic>;
  using Param = param_mixture_t<Mat, param_tag_dense>;

  Param ret(n1, n2, opt);
  param_impl_initialize(ret, param_tag_mixture());
  param_resolve(ret);
  hyperparam_resolve(ret);
...
template <typename scalar_t, typename Index, typename Opt>
auto
make_dense_mixture_ptr(const Index n1, const Index n2, const Opt& opt) {
  using Mat   = Eigen::Matrix<scalar_t, Eigen::Dynamic, Eigen::Dynamic>;
  using Param = param_mixture_t<Mat, param_tag_dense>;
...
auto
make_sparse_mixture(const Eigen::SparseMatrixBase<Derived>& A, const Opt& opt) {
  const auto n1 = A.rows();
  const auto n2 = A.cols();

  using Mat   = Eigen::SparseMatrix<scalar_t, Eigen::ColMajor>;
  using Param = param_mixture_t<Mat, param_tag_sparse>;
...
void
param_impl_update_sgd(Parameter& P, const scalar_t rate, const param_tag_mixture) {
  ...
  P.gamma_null_aux += update_adam(P.adam_gamma_null_aux, P.grad_gamma_null_aux) * rate;
  param_resolve(P);
}

template <typename Parameter, typename scalar_t>
void
hyperparam_impl_update_sgd(Parameter& P, const scalar_t rate, const param_tag_mixture) {
  P.pi_aux += update_adam(P.adam_pi_aux, P.grad_pi_aux) * rate;
  P.tau_aux += update_adam(P.adam_tau_aux, P.grad_tau_aux) * rate;
  P.tau_null_aux += update_adam(P.adam_tau_null_aux, P.grad_tau_null_aux) * rate;
  hyperparam_resolve(P);
}

// mean and variance
template <typename Parameter>
void
param_impl_resolve(Parameter& P, const param_tag_mixture) {
  ...
  P.theta_var = P.omega_null + P.alpha.cwiseProduct(P.omega + P.beta.cwiseProduct(P.beta)) -
                P.alpha.cwiseProduct(P.beta).cwiseProduct(P.alpha.cwiseProduct(P.beta));
}

template <typename Parameter>
void
hyperparam_impl_resolve(Parameter& P, const param_tag_mixture) {
...
void
param_impl_perturb(Parameter& P, const scalar_t sd, RNG& rng, const param_tag_mixture) {
...
template <typename Parameter, typename scalar_t>
void
param_impl_perturb(Parameter& P, const scalar_t sd, const param_tag_mixture) {
  std::mt19937 rng;
  param_impl_perturb(P, sd, rng, param_tag_mixture());
}

template <typename Parameter>
void
param_impl_check_nan(Parameter& P, const param_tag_mixture) {
...
template <typename Parameter>
const auto&
param_impl_mean(Parameter& P, const param_tag_mixture) {
  return P.theta;
}

template <typename Parameter>
auto
param_impl_log_odds(Parameter& P, const param_tag_mixture) {
  return P.alpha_aux.unaryExpr([&P](const auto& x) { return P.pi_aux + x; });
}

template <typename Parameter>
const auto&
param_impl_var(Parameter& P, const param_tag_mixture) {
  return P.theta_var;
}
...
param_impl_eval_sgd(Parameter& P, const M1& G1, const M2& G2, const M3& Nobs,
  ...
  P.grad_alpha_aux = G1.cwiseProduct(P.beta) +
                     ...
                     0.5 * (P.omega.binaryExpr(P.omega_null, P.grad_ln_ratio_alpha_op) +
  ...
  P.grad_gamma_aux = P.alpha.cwiseProduct(G2 + P.omega.binaryExpr(P.omega_null, P.grad_kl_gamma_op))
  ...
  P.grad_gamma_null_aux =
      (G2 + P.alpha.binaryExpr(P.omega, P.grad_kl_gamma_null_op) +
       P.alpha.cwiseProduct(P.omega.binaryExpr(P.omega_null, P.grad_kl_gamma_op)))
...
template <typename Parameter, typename M1, typename M2, typename M3>
void
hyperparam_impl_eval_sgd(Parameter& P, const M1& G1, const M2& G2, const M3& Nobs,
                         const param_tag_mixture) {
  using scalar_t      = typename Parameter::scalar_t;
  ...
  param_impl_eval_sgd(P, G1, G2, Nobs, param_tag_mixture());
  ...
  const scalar_t stuff =
      P.alpha.binaryExpr(P.beta.cwiseProduct(P.beta) + P.omega + P.omega_null, P.grad_kl_tau_op)
          .sum();

  P.grad_tau_aux = -P.grad_gamma_aux.sum() + stuff * P.grad_tau_chain_op(P.tau_aux);
  ...
  const scalar_t stuff0 = P.alpha.binaryExpr(P.omega_null, P.grad_kl_tau_null_op).sum();

  P.grad_tau_null_aux =
      -P.grad_gamma_null_aux.sum() + (stuff + stuff0) * P.grad_tau_chain_op(P.tau_null_aux);

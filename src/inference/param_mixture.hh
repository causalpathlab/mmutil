...
struct param_mixture_t {
    ...
    struct resolve_var_op_t {
        explicit resolve_var_op_t(const scalar_t _gammax,
    ...
    struct grad_kl_lodds_alpha_t {
        explicit grad_kl_lodds_alpha_t(const scalar_t &_lodds)
            : lodds(_lodds)
        {
        }
    ...
    struct grad_kl_gamma_null_t {
        explicit grad_kl_gamma_null_t(const scalar_t &_v0)
            : v0(_v0)
        {
        }
        inline const scalar_t operator()(const scalar_t &a,
    ...
    grad_gamma_chain_rule_t
        grad_gamma_chain_op; // grad_gamma_op(gammax,tau_aux)
    grad_gamma_chain_rule_t
    ...
    struct grad_kl_tau_null_t {
        explicit grad_kl_tau_null_t(const scalar_t &_v0)
            : v0(_v0)
        {
        }
...
hyperparam_impl_resolve(Parameter &P, const param_tag_mixture)
{
    ...
    if (P.tau_null_aux > P.tau_lodds_ub)
        P.tau_null_aux = P.tau_lodds_ub;
    if (P.tau_null_aux < P.tau_lodds_lb)
        P.tau_null_aux = P.tau_lodds_lb;
    ...
    P.grad_alpha_aux = G1.cwiseProduct(P.beta) +
        ...
        0.5 *
            (P.omega.binaryExpr(P.omega_null, P.grad_ln_ratio_alpha_op) +
             ...
             (P.beta.cwiseProduct(P.beta) + P.omega + P.omega_null) /
                 var_prior);
    ...
    P.grad_gamma_null_aux =
        (G2 + P.alpha.binaryExpr(P.omega, P.grad_kl_gamma_null_op) +
         P.alpha.cwiseProduct(
             P.omega.binaryExpr(P.omega_null, P.grad_kl_gamma_op)))
            .cwiseProduct(
                P.gamma_null_aux.unaryExpr(P.grad_gamma_chain_null_op));

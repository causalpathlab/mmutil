...
struct param_row_spike_slab_t {
  ...
  typedef param_tag_row_spike_slab sgd_tag;
  typedef S sparsity_tag;

  using Dense = Eigen::Matrix<scalar_t, Eigen::Dynamic, Eigen::Dynamic>;

  template <typename Opt>
  explicit param_row_spike_slab_t(const index_t n1, const index_t n2, const Opt& opt)
      : nrow(n1),
        ncol(n2),
        onesN2(n2, 1),
        alpha(nrow, ncol),
  ...
  ////////////////////////////////////////////////////////////////
  // adaptive gradient
  grad_adam_t adam_alpha_aux_row;
  grad_adam_t adam_beta;
  grad_adam_t adam_gamma_aux_row;
  ...
  struct grad_alpha_beta_gamma_tau_t {
    explicit grad_alpha_beta_gamma_tau_t(const scalar_t& _tau_val) : tau_val(_tau_val) {}
    inline const scalar_t operator()(const scalar_t& b, const scalar_t& g) const {
      return half_val *
             (one_val + std::log(tau_val) - std::log(g) - tau_val * (b * b + one_val / g));
    }
  ...
  struct grad_beta_eb_tau_t {
    ...
    inline const scalar_t operator()(const scalar_t& b) const { return tau_val * b; }
    const scalar_t& tau_val;
  } grad_beta_eb_tau_op;
  ...
  struct grad_gamma_chain_rule_t {
    ...
    inline const scalar_t operator()(const scalar_t& g_aux) const {
      ...
      if (lo > large_exp_value) return std::exp(-lo) / (one_val + std::exp(-lo));
      return one_val / (one_val + std::exp(lo));
    }
    const scalar_t& tau_aux;
  ...
  struct grad_prior_tau_aux_t {
    explicit grad_prior_tau_aux_t(const scalar_t& _tau_val, const scalar_t& _tau_aux)
        : tau_val(_tau_val),
          tau_aux(_tau_aux) {}
    inline const scalar_t operator()(const scalar_t& g, const scalar_t& b) const {
...
void
param_impl_initialize(Parameter& P, const param_tag_row_spike_slab) {
  setConstant(P.onesN2, 1.0);
  ...
  setConstant(P.alpha_aux_row, 0.0);
  P.grad_pi_aux = 0.0;

  // complementary to each other
  setConstant(P.gamma_aux, P.tau_lodds_lb);
  setConstant(P.gamma_aux_row, P.tau_lodds_lb);
...
auto
make_dense_row_spike_slab(const Index n1, const Index n2, const Opt& opt) {
  using Mat   = Eigen::Matrix<Scalar, Eigen::Dynamic, Eigen::Dynamic>;
  using Param = param_row_spike_slab_t<Mat, param_tag_dense>;

  Param ret(n1, n2, opt);
  param_impl_initialize(ret, param_tag_row_spike_slab());
  param_resolve(ret);
...
template <typename Scalar, typename Index, typename Opt>
auto
make_dense_row_spike_slab_ptr(const Index n1, const Index n2, const Opt& opt) {
  using Mat   = Eigen::Matrix<Scalar, Eigen::Dynamic, Eigen::Dynamic>;
  using Param = param_row_spike_slab_t<Mat, param_tag_dense>;
...
auto
make_sparse_row_spike_slab(const Eigen::SparseMatrixBase<Derived>& A, const Opt& opt) {
  const auto n1 = A.rows();
  const auto n2 = A.cols();

  using Mat   = Eigen::SparseMatrix<Scalar, Eigen::ColMajor>;
  using Param = param_row_spike_slab_t<Mat, param_tag_sparse>;
  ...
  Mat Acol    = (Dense::Ones(1, n1) * A).unaryExpr([](const auto& x) { return 1.0; });
  ...
  initialize(Acol, ret.alpha_aux_row, eps);
  initialize(Acol, ret.gamma_aux_row, eps);
  initialize(Acol, ret.grad_alpha_aux_row, eps);
  initialize(Acol, ret.grad_gamma_aux_row, eps);
...
void
param_impl_update_sgd(Parameter& P, const Scalar rate, const param_tag_row_spike_slab) {
  using scalar_t       = typename Parameter::scalar_t;
  const scalar_t denom = static_cast<scalar_t>(P.cols());

  P.grad_alpha_aux_row = P.grad_alpha_aux * P.onesN2 / denom;
  P.grad_gamma_aux_row = P.grad_gamma_aux * P.onesN2 / denom;

  P.alpha_aux_row += update_adam(P.adam_alpha_aux_row, P.grad_alpha_aux_row) * rate;
  P.gamma_aux_row += update_adam(P.adam_gamma_aux_row, P.grad_gamma_aux_row) * rate;
...
template <typename Parameter, typename Scalar>
void
hyperparam_impl_update_sgd(Parameter& P, const Scalar rate, const param_tag_row_spike_slab) {
...
template <typename Parameter>
void
param_impl_resolve(Parameter& P, const param_tag_row_spike_slab) {
  for (auto c = 0; c < P.cols(); ++c) {
    P.alpha_aux.col(c) = P.alpha_aux_row;
    P.gamma_aux.col(c) = P.gamma_aux_row;
  }
...
template <typename Parameter>
void
hyperparam_impl_resolve(Parameter& P, const param_tag_row_spike_slab) {
...
template <typename Parameter, typename Scalar>
void
param_impl_perturb(Parameter& P, const Scalar sd, const param_tag_row_spike_slab) {
  std::mt19937 rng;
...
template <typename Parameter>
void
param_impl_check_nan(Parameter& P, const param_tag_row_spike_slab) {
...
template <typename Parameter>
const auto&
param_impl_mean(Parameter& P, const param_tag_row_spike_slab) {
  return P.theta;
}

template <typename Parameter>
auto
param_impl_log_odds(Parameter& P, const param_tag_row_spike_slab) {
  return P.alpha_aux;
}

template <typename Parameter>
const auto&
param_impl_var(Parameter& P, const param_tag_row_spike_slab) {
  return P.theta_var;
}
...
param_impl_eval_sgd(Parameter& P, const M1& G1, const M2& G2, const M3& Nobs,
  ...
  P.grad_alpha_aux += P.beta.binaryExpr(P.gamma, P.grad_alpha_beta_gamma_tau_op);
  ...
  // gradient w.r.t. beta
  P.grad_beta =
      G1.cwiseProduct(P.alpha) + G2.cwiseProduct(P.alpha.binaryExpr(P.beta, P.grad_beta_g2_op));
...
param_impl_write(Parameter& P, const std::string hdr, const std::string gz,
  ...
  typename Parameter::data_t temp =
      P.alpha_aux_row.unaryExpr([&P](const auto& x) { return P.pi_aux + x; });
  write_data_file((hdr + ".lodds" + gz), temp);
  write_data_file((hdr + ".slab" + gz), P.beta);
  write_data_file((hdr + ".slab_prec" + gz), P.gamma);

  TLOG("Pi = " << std::setw(10) << P.pi_val << " [" << std::setw(10) << P.pi_aux << "] Tau = "
               << std::setw(10) << P.tau_val << " [" << std::setw(10) << P.tau_aux << "]");
}

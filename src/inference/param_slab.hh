...
struct param_slab_t {
  ...
  typedef param_tag_slab sgd_tag;
...
void
param_impl_initialize(Parameter& P, const param_tag_slab) {
...
make_dense_slab(const Index n1, const Index n2, const Opt& opt) {
  ...
  using Param = param_slab_t<Mat, param_tag_dense>;
  Param ret(n1, n2, opt);
  param_impl_initialize(ret, param_tag_slab());
  param_resolve(ret);
...
make_sparse_slab(const Eigen::SparseMatrixBase<Derived>& A, const Opt& opt) {
  ...
  using Param = param_slab_t<Mat, param_tag_sparse>;
  Param ret(n1, n2, opt);
...
template <typename Parameter>
void
param_impl_resolve(Parameter& P, const param_tag_slab) {
...
template <typename Parameter>
void
hyperparam_impl_resolve(Parameter& P, const param_tag_slab) {
...
void
param_impl_perturb(Parameter& P, const Scalar sd, RNG& rng, const param_tag_slab) {
...
void
param_impl_perturb(Parameter& P, const scalar_t sd, const param_tag_slab) {
  std::mt19937 rng;
  param_impl_perturb(P, sd, rng, param_tag_slab());
}

template <typename Parameter>
const auto&
param_impl_mean(Parameter& P, const param_tag_slab) {
  return P.beta;
}

template <typename Parameter>
const auto&
param_impl_var(Parameter& P, const param_tag_slab) {
  return P.theta_var;
}
...
void
param_impl_update_sgd(Parameter& P, const Scalar rate, const param_tag_slab) {
...
template <typename Parameter, typename Scalar>
void
hyperparam_impl_update_sgd(Parameter& P, const Scalar rate, const param_tag_slab) {
...
template <typename Parameter>
void
param_impl_write(Parameter& P, const std::string hdr, const std::string gz, const param_tag_slab) {
...
template <typename Parameter>
void
param_impl_check_nan(Parameter& P, const param_tag_slab) {

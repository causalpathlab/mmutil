...
build_boost_graph(const Data& data, const Str2Int& vertex2index, Graph& G) {
  ...
  auto add_edge = [&G](const auto& pp) {
    ...
    if (!has_edge) boost::add_edge(pp.first, pp.second, G);
  };

  for_each(data.begin(), data.end(), update_max_vertex);

  // add vertices
  for (auto u = boost::num_vertices(G); u <= max_vertex; ++u) boost::add_vertex(G);
...
prune_uninformative_edges(const Graph& gIn, Graph& gOut, const Scalar snCutoff) {
  ...
  for (boost::tie(ei, eEnd) = boost::edges(gIn); ei != eEnd; ++ei) {
    ...
    // bool has_sn = false;
    Scalar sn = 0.0;
    for (boost::tie(ani, anEnd) = boost::adjacent_vertices(a, gIn); ani != anEnd; ++ani) {
      const typename Graph::vertex_descriptor an = *ani;
      for (boost::tie(bni, bnEnd) = boost::adjacent_vertices(b, gIn); bni != bnEnd; ++bni) {
    ...
    if (sn >= snCutoff) boost::add_edge(a, b, gOut);
  }
}
...
read_network_data(const std::string data_file, const std::string color_file = "",
  ...
  auto make_component = [&color](auto& data) {
    ...
    TLOG("constructed " << comp.A.rows() << " x " << comp.A.cols() << " adjacency matrix");
